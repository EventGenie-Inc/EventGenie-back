...
const router = Router();
router.get('/current', authenticate, requireEventAdmin, async (req, res, next) => {
    try {
        const auth = req;
...
router.put('/current', authenticate, requireEventAdmin, async (req, res, next) => {
    try {
        const auth = req;
        const tenantId = auth.user.tenantId;
        if (!tenantId) {
...
router.delete('/current', authenticate, requireEventAdmin, async (req, res, next) => {
    try {
        const auth = req;
        const tenantId = auth.user.tenantId;
        if (!tenantId) {
...
router.post('/current/materialize', authenticate, requireEventAdmin, async (req, res, next) => {
    try {
        const auth = req;
        const tenantId = auth.user.tenantId;
        if (!tenantId) {
...
//# sourceMappingURL=event-draft.router.js.map