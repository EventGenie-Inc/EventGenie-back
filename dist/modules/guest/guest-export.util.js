...
const slugify = (name) => name
    ...
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '');
const displayName = (guest) => [guest.firstName, guest.surname].filter(Boolean).join(' ').trim() || 'Guest';
...
// any row is built.
const groupByHost = (guests) => {
    const primaries = guests.filter((g) => !g.hostGuestId);
    const plusOnesByHost = new Map();
    for (const guest of guests) {
        if (!guest.hostGuestId)
            continue;
...
export const buildGuestExportWorkbook = async (event, eventDays, rsvpFields, guests) => {
...
//# sourceMappingURL=guest-export.util.js.map