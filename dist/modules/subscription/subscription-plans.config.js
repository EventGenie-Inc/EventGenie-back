...
export const SUBSCRIPTION_PRICES_CENTS = {
...
export const isPaidTier = (tier) => tier === 'CELEBRATE' || tier === 'ELEVATE';
...
export const getPlanCode = (tier, period) => {
    const envVar = PLAN_ENV_VAR[tier][period];
    const code = process.env[envVar];
    if (!code) {
        throw new Error(`${envVar} is not defined in .env — run prisma/create-subscription-plans.ts to create the Paystack plans for this environment, then set the resulting codes.`);
    }
    return code;
};
...
export const resolveTierAndPeriodFromPlanCode = (planCode) => {
    for (const tier of ['CELEBRATE', 'ELEVATE']) {
