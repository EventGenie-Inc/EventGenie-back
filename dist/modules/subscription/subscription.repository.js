...
export const subscriptionRepository = {
    findBillingStateByTenantId: (tenantId, db = prisma) => db.tenant.findFirst({ where: { id: tenantId }, select: SUBSCRIPTION_FIELDS }),
    // Renewal / upgrade charges carry no reference we chose — this is how
    // they're matched back to a tenant. @unique on the column.
    findByCustomerCode: (customerCode, db = prisma) => db.tenant.findFirst({ where: { paystackCustomerCode: customerCode }, select: SUBSCRIPTION_FIELDS }),
    ...
    findByEmail: (email, db = prisma) => db.tenant.findFirst({ where: { email }, select: SUBSCRIPTION_FIELDS }),
    ...
    findByPendingReference: (reference, db = prisma) => db.tenant.findFirst({ where: { subscriptionPendingReference: reference }, select: SUBSCRIPTION_FIELDS }),
    savePendingAttempt: (tenantId, data, db = prisma) => db.tenant.update({
        where: { id: tenantId },
        data: {
    ...
    clearPendingAttempt: (tenantId, db = prisma) => db.tenant.update({
        where: { id: tenantId },
        data: {
    ...
    recordChargeSucceededPreliminary: (tenantId, data, db = prisma) => db.tenant.update({
        where: { id: tenantId },
        data: {
    ...
    activateSubscription: (tenantId, data, db = prisma) => db.tenant.update({
        where: { id: tenantId },
        data: {
    ...
    updateCurrentPeriodEnd: (tenantId, currentPeriodEnd, db = prisma) => db.tenant.update({ where: { id: tenantId }, data: { subscriptionCurrentPeriodEnd: currentPeriodEnd } }),
    ...
    recordChargeFailureIfFirst: (tenantId, db = prisma) => db.$executeRaw `
      UPDATE "Tenant"
    ...
    scheduleEndOfPeriodChange: (tenantId, pendingTier, db = prisma) => db.tenant.update({
        where: { id: tenantId },
        data: {
            subscriptionCancelAtPeriodEnd: true,
