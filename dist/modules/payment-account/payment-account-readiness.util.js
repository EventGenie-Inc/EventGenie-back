...
// ─────────────────────────────────────────
export const assertEventReadyToSellTickets = async (event, db = prisma) => {
    ...
    if (resolveEventEntitlement(tenant, event) === 'SPARK') {
        throw new HttpError(403, 'The SPARK plan does not support paid ticketing. Upgrade to CELEBRATE or ELEVATE, or buy an Event Pass for this event, to sell tickets.');
    }
    if (tenant.paystackSubaccountStatus !== 'ACTIVE' || !tenant.paystackSubaccountCode) {
        throw new HttpError(422, 'Paid ticketing needs an active payout account before it can be used. Add your bank details under payment settings first.');
    }
...
//  solve it.
// ─────────────────────────────────────────
export const assertSubaccountReadyForPurchase = async (tenantId, db = prisma) => {
    const tenant = await paymentAccountRepository.findStatusByTenantId(tenantId, db);
    if (!tenant)
        throw new HttpError(404, 'Tenant not found');
    if (tenant.paystackSubaccountStatus !== 'ACTIVE' || !tenant.paystackSubaccountCode) {
        throw new HttpError(422, "This event's payout account is not currently active, so ticket purchases can't be completed right now. Please contact the organiser.");
    }
    return { subaccountCode: tenant.paystackSubaccountCode };
};
