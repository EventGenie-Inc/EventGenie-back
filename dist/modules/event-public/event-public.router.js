...
import { publicEventViewLimiter, publicRegistrationIpLimiter, publicRegistrationEventLimiter, } from '../../shared/middleware/rate-limit.middleware.js';
...
router.get('/:shareToken', publicEventViewLimiter, async (req, res, next) => {
    try {
        const result = await eventPublicService.viewByShareToken(req.params['shareToken']);
...
router.post('/:shareToken/register', publicRegistrationIpLimiter, publicRegistrationEventLimiter, async (req, res, next) => {
    try {
        const result = await eventPublicService.register(req.params['shareToken'], req.body);
...
//# sourceMappingURL=event-public.router.js.map