...
import { normalizeEmail, assertValidEmail, normalizePhoneToE164, assertExactlyOneContact, findDuplicateContact, } from '../guest/guest-validation.util.js';
import {} from './event-public.types.js';
...
const REGISTRATION_BLOCK_MESSAGES = {
    ...RSVP_BLOCK_MESSAGES,
...
const assertEventAcceptsRegistration = (event) => {
    ...
    if (event.rsvpDeadline && event.rsvpDeadline < new Date()) {
        throw new HttpError(410, `Registration for this event closed on ${formatGuestDate(event.rsvpDeadline)}. Contact the organiser if you still need to attend.`);
    }
};
...
export const eventPublicService = {
    ...
    register: async (shareToken, data) => {
