...
import { EVENT_COVER_ALLOWED_FORMATS, EVENT_COVER_MAX_BYTES } from './upload-constants.js';
...
//  instead enforced after the fact, at event-save time, in
//  event/event-cover-image.util.ts + event.service.ts — see the batch
//  report for the full reasoning.
// ─────────────────────────────────────────
export const uploadService = {
    requestSignature: async (requestingRole, requestingTenantId, data) => {
        if (data.purpose === 'MEMORY_ITEM') {
            throw new HttpError(400, 'Memory Hub uploads are not available yet.');
        }
        if (data.purpose !== 'EVENT_COVER') {
            throw new HttpError(400, `Unknown upload purpose '${data.purpose}'`);
        }
        ...
        const signature = signCloudinaryParams({ allowed_formats: EVENT_COVER_ALLOWED_FORMATS, folder, public_id: publicId, timestamp }, apiSecret);
        return {
            signature,
            timestamp,
            apiKey,
