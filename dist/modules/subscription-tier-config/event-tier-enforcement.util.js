...
export const assertEventCreatable = async (tenantId, input) => {
    ...
    if (config?.maxEvents != null) {
        ...
        if (activeCount >= config.maxEvents) {
            throw new HttpError(403, `The ${effectiveTier} plan allows a maximum of ${config.maxEvents} active event(s). Archive an existing event or upgrade your plan to create another.`);
        }
    }
};
