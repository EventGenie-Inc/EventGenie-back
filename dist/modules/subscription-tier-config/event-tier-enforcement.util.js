...
const assertSparkCapabilityGates = (tier, input) => {
    if (tier !== 'SPARK')
        return;
...
export const assertEventCreatable = async (tenantId, input) => {
    ...
    if (config?.maxEvents != null) {
        ...
        if (activeCount >= config.maxEvents) {
            throw new HttpError(403, `The ${tenant.subscriptionTier} plan allows a maximum of ${config.maxEvents} active event(s). Archive an existing event or upgrade your plan to create another.`);
        }
    }
};
// Called before updating an existing event (PUT /api/events/:id). No
// maxEvents check here — an update doesn't create a new event.
export const assertEventUpdatable = async (tenantId, input) => {
    const tenant = await tenantRepository.findById(tenantId);
    if (!tenant)
        throw new HttpError(404, 'Tenant not found');
    assertSparkCapabilityGates(tenant.subscriptionTier, input);
};
//# sourceMappingURL=event-tier-enforcement.util.js.map