...
export interface TicketChargeBreakdown {
  // What the organiser receives — always exactly the ticket's own
  // price × quantity, untouched by any of the arithmetic below.
  ticketPriceCents: number;
  // EventGenie's target commission — 2.5% (or whatever
  // PLATFORM_COMMISSION_PERCENT currently is) of ticketPriceCents.
  commissionCents: number;
  // What the GUEST is actually charged — this is the `amount` sent to
  // Paystack's Initialize Transaction.
  totalChargeCents: number;
  // What is routed to the MAIN (platform) account via Paystack's
  // `transaction_charge` override — commission + Paystack's own fee.
  // The organiser's subaccount receives totalChargeCents minus this,
  // which by construction is always exactly ticketPriceCents: Paystack
  // computes the subaccount's share as `amount - transaction_charge`,
  // so pinning transaction_charge to (T - ticketPrice) rather than to a
  // separately-recomputed (commission + fee) is what GUARANTEES the
  // organiser's exact price survives untouched even if the fee estimate
  // below is off by a cent — any rounding slop lands on the platform's
  // side, never the organiser's.
...
//  worth solving.
// ─────────────────────────────────────────
export const computeTicketChargeCents = (ticketPriceCents: number, quantity: number): TicketChargeBreakdown => {
