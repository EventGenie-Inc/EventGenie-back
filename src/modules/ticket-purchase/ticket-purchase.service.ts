...
const generatePaymentReference = (): string => `tkt_${crypto.randomBytes(16).toString('hex')}`;
const nextHoldExpiry = (): Date => new Date(Date.now() + HOLD_DURATION_MINUTES * 60_000);

export interface ReserveTicketPurchaseInput {
  tenantId: string;
  eventId: string;
  inviteId: string;
  ticket: { id: string; price: Prisma.Decimal; currency: string };
  quantity: number;
}
...
export interface StartCheckoutInput {
  // Identifies which purchase to mark FAILED if this call errors — via
  // paymentRef (findByPaymentRef), not purchaseId directly, so
  // failPayment stays the single entry point regardless of caller.
  paymentRef: string;
  totalChargeCents: number;
  platformChargeCents: number;
  guestEmail: string;
...
export const ticketPurchaseService = {
  ...
  getAllForEvent: (eventId: string) => ticketPurchaseRepository.findAllForEvent(eventId),
  ...
  reserveWithinTransaction: async (
    ...
    if (!secured) {
      throw new HttpError(
        409,
        "There aren't enough tickets left for the quantity you selected. Try a smaller quantity or contact the organiser."
      );
    }
  ...
  startPaystackCheckout: async (params: StartCheckoutInput): Promise<CheckoutResult> => {
    try {
  ...
  retryPayment: async (
    ...
    | { status: 'PAID' | 'PENDING' }
    | { status: 'RETRYING'; paymentRef: string; totalChargeCents: number; platformChargeCents: number; subaccountCode: string }
  > => {
    const outcome = await prisma.$transaction(async (tx) => {
      const locked = await ticketPurchaseRepository.findByIdForUpdate(input.purchaseId, tx);
      if (!locked) throw new HttpError(404, 'Ticket purchase not found');

      if (locked.status === 'PAID') return { status: 'PAID' as const };
      ...
      await ticketPurchaseRepository.resetForRetry(
        input.purchaseId,
        {
          paymentRef,
          holdExpiresAt,
  ...
  // own page.
  reconcile: async (purchaseId: string): Promise<{ status: 'PENDING' | 'PAID' | 'FAILED' | 'EXPIRED' }> => {
    ...
    if (verified.status === 'success') {
      await ticketPurchaseService.confirmPayment(purchase.paymentRef, verified.raw as unknown as Prisma.InputJsonValue);
      return { status: 'PAID' };
    }
  ...
  // confirmation.
  confirmPaymentWithinTransaction: async (
    tx: Prisma.TransactionClient,
    paymentRef: string,
    ...
    if (locked.status === 'PAID') return { outcome: 'already_handled' };
  ...
  confirmPayment: (
    ...
    prisma.$transaction((tx) => ticketPurchaseService.confirmPaymentWithinTransaction(tx, paymentRef, providerPayload), NEON_TRANSACTION_OPTIONS),
  ...
  failPaymentWithinTransaction: async (
    tx: Prisma.TransactionClient,
    paymentRef: string,
    providerPayload: Prisma.InputJsonValue
    ...
    const locked = await ticketPurchaseRepository.findByIdForUpdate(existing.id, tx);
    if (!locked) return { outcome: 'not_found' };
    if (locked.status !== 'PENDING') return { outcome: 'already_handled' };
  ...
  failPayment: (
    ...
    prisma.$transaction((tx) => ticketPurchaseService.failPaymentWithinTransaction(tx, paymentRef, providerPayload), NEON_TRANSACTION_OPTIONS),
};
