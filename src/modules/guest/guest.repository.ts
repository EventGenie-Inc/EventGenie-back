...
export const guestRepository = {
  ...
  findAllForEvent: (eventId: string, includeArchived = false) =>
    prisma.guest.findMany({
      where: {
        eventId,
        ...(includeArchived ? {} : { isArchived: false }),
    ...
    prisma.$transaction(async (tx) => {
      ...
      const invite = await tx.invite.create({
        data: {
          ...
          status: 'PENDING',
          used: false,
          deliveryMethod: (data.email ? 'EMAIL' : 'SMS') as DeliveryMethod,
          expiresAt: null,
          isArchived: false,
          createdBy: userId,
          updatedBy: userId,
    ...
    prisma.$transaction(async (tx) => {
      ...
      await tx.invite.createMany({
        data: rows.map((row, i) => ({
          ...
          status: 'PENDING',
          used: false,
          deliveryMethod: (row.email ? 'EMAIL' : 'SMS') as DeliveryMethod,
          expiresAt: null,
          isArchived: false,
          createdBy: userId,
          updatedBy: userId,
