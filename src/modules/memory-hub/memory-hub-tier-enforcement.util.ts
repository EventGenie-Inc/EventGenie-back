...
export const assertMemoryHubAccessible = async (tenantId: string): Promise<void> => {
  ...
  if (!config?.memoryHubEnabled) {
    throw new HttpError(
      403,
      `The ${tenant.subscriptionTier} plan does not include Memory Hub. Upgrade to CELEBRATE or ELEVATE to use it.`
    );
  }
};

// null = unlimited, same convention as every other max* column.
export const getMemoryHubQuotaBytes = async (tenantId: string): Promise<number | null> => {
  const tenant = await tenantRepository.findById(tenantId);
  if (!tenant) throw new HttpError(404, 'Tenant not found');
  const config = await subscriptionTierConfigRepository.findByTier(tenant.subscriptionTier);
...
// ─────────────────────────────────────────
export const assertMemoryHubQuotaAvailable = async (eventId: string, tenantId: string): Promise<void> => {
  ...
  if (usedBytes >= maxBytes) {
    ...
    throw new HttpError(
      403,
      `This event's Memory Hub has reached its ${Math.round(maxBytes / (1024 * 1024))}MB storage limit for the ${tenant?.subscriptionTier ?? 'current'} plan. Remove items or upgrade the plan to add more.`
    );
  }
};
