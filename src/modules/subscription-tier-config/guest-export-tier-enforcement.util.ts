...
export const assertGuestExportEnabled = async (tenantId: string): Promise<void> => {
  ...
  if (!config?.guestExportEnabled) {
    throw new HttpError(
      403,
      `The ${tenant.subscriptionTier} plan does not include guest list export. Upgrade to CELEBRATE or ELEVATE to use it.`
    );
  }
};
