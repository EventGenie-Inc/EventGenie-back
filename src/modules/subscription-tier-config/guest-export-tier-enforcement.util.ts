...
//
// EVENT-SCOPED (Event Pass batch) — "exports one event's guests," per
// the batch prompt's own classification. Takes the full fetched event,
// not a bare tenantId, so a missed call site fails to compile rather
// than falling back to tenant-only resolution.
//
...
export const assertGuestExportEnabled = async (event: EntitlementDerivableEvent): Promise<void> => {
  ...
  if (!config?.guestExportEnabled) {
    throw new HttpError(
      403,
      `The ${effectiveTier} plan does not include guest list export. Upgrade to CELEBRATE or ELEVATE to use it.`
    );
  }
};
